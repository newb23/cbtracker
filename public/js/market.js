...
var marketEvent = _.find(NFTMarket, (i) => i.name === "PurchasedListing");
...
function init() {
  ...
  wssWeb3 = new Web3(
    new Web3.providers.WebsocketProvider(wssNodes[currentNetwork], options)
  );
  ...
  if (sales.length > 0) {
    sales.forEach((data) => {
      marketResult.append(
        `${data.type},${data.id},${data.price},${data.seller},${data.date},${data.txHash}\n`
      );
    });
  }
    ...
    if (
      ...
      if (subs.includes(returnValues.seller)) {
        if (!sales.find((i) => i.transactionHash === data.transactionHash)) {
          ...
          marketResult.append(
            `${type},${nftID},${realPrice},${seller},${date},${data.transactionHash}\n`
          );
          notify(
            "Market Sales Tracker",
            `${ucfirst(type)} #${nftID} sold for ${realPrice} SKILL`
          );
          storeSales({
            type,
            seller,
...
function storeSales(data) {
  sales.push(data);
  if (sales)
    localStorage.setItem(`sales-${currentNetwork}`, JSON.stringify(sales));
}
