...
var isGen2 = (localStorage.getItem('gen2') === 'true')
...
var isPhone = $('body').data('is-phone')
...
if (isGen2) {
  $('#btn-gen2').prop('checked', true)
} else {
  $('#btn-gen2').removeAttr('checked')
}
...
async function priceTicker() {
  ...
  $.get(`https://api.coingecko.com/api/v3/simple/price?ids=tether,near,cryptoblades,coinex-token&vs_currencies=${currencies.join(',')}`, async (result) => {
    ...
    if (currentNetwork === 'bnb') {
      if (isGen2) {
        skillPrice = 0 // temporary 0 price for valor token
      } else {
        skillPrice *= gasPrice
      }
    }
...
function gen2Helper(toggle) {
  if (toggle && currentNetwork === 'bnb') {
    ...
    $('#card-unclaimed-title').html('Total Unclaimed Valors')
    $('#card-staked-title').html('Total Staked Valors')
    $('#card-wallet-title').html('Total Valors in Wallet')
    $('#card-total-title').html('Total Valor Assets')
    $('#card-price-title').html('Current Valor Price')
    $('#card-multiplier-title').html('Valor Multiplier')
  } else {
    $('#label-asset').html('Skill Assets')
    $('#btn-hskill-label').html('Hide skill assets')
    $('#card-claimable-title').html('Total Claimable Skills')
    $('#card-unclaimed-title').html('Total Unclaimed Skills')
    $('#card-staked-title').html('Total Staked Skills')
...
$("#btn-gen2").on('change', (e) => {
  isGen2 = e.currentTarget.checked
  localStorage.setItem('gen2', isGen2)
  if (currentNetwork === 'bnb') {
    gen2Helper(isGen2)
    clearFiat()
    priceTicker()
